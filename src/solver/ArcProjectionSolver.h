...
struct GrindingWheelParameters
{
    ...
    // 默认为1A1砂轮
    std::function<double(double)> radius{[](double u0) { return 50.; }};
    std::function<double(double)> radiusDeriv{[](double u0) { return 0.; }};
};
...
struct CutterParameters
{
    ...
    std::function<double(double)> coreRadius{[](double u1) { return 2.5; }};

    std::function<double(double)> radialRakeAngle{[](double u1) { return glm::radians(10.); }};  // 径向前角 
...
private:
    ...
    // Eq.8 切削刃曲线单位法向量nt_1
    glm::dvec4 CalculateCuttingEdgeCurveNormal(double u1, double theta1, const glm::dvec4& r_t1);
    // Eq.9 计算法向前角
    double CalculateNormalRakeAngle(double u1, const glm::dvec4& r_t1, const glm::dvec4& b_t,
                                    const glm::dvec4& m_t);
