...
void OptimizeResourceContext::CreateSampler()
{
...
void OptimizeResourceContext::CreateGlobalResources()
{
    m_cameraSSBOBuffer = std::make_unique<LveBuffer>(
        m_lveDevice,
        sizeof(CameraData),
        BATCH_LAYER_COUNT,
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    m_cameraSSBOBuffer->Map();
    ...
    m_globalPool = LveDescriptorPool::Builder(m_lveDevice)
                       ...
                       .Build();
    auto bufferInfo = m_cameraSSBOBuffer->DescriptorInfo();
...
void OptimizeResourceContext::CreateRenderPass()
{
...
void OptimizeResourceContext::CreateOffscreenImage()
{
    // 棒料资源
    CreateImageResources(m_blankMaskImage,
                         m_blankMaskMemory,
                         m_blankMaskView,
                         m_blankMaskFramebuffer);
    // 砂轮资源
    CreateImageResources(m_grndWheelMaskImage,
                         m_grndWheelMaskMemory,
                         m_grndWheelMaskView,
                         m_grndWheelMaskFramebuffer);
    // 交集资源
    CreateImageResources(m_contactMaskImage,
                         m_contactMaskMemory,
                         m_contactMaskView,
    ...
    imageInfo.arrayLayers = BATCH_LAYER_COUNT;
    ...
    viewInfo.subresourceRange.layerCount = BATCH_LAYER_COUNT;
    ...
    auto transitionImage =
        [&](VkImage img, VkImageAspectFlags aspect, VkImageLayout newLayout) {
            ...
            barrier.subresourceRange.layerCount = BATCH_LAYER_COUNT;
...
void OptimizeResourceContext::CreateImageResources(VkImage& image, VkDeviceMemory& memory,
                                                VkImageView& view,
...
void OptimizeResourceContext::CreateFramebuffers()
{
    CreateFramebuffers(m_blankMaskView, m_blankMaskFramebuffer);
    CreateFramebuffers(m_grndWheelMaskView, m_grndWheelMaskFramebuffer);
    CreateFramebuffers(m_contactMaskView, m_contactMaskFramebuffer);
}

void OptimizeResourceContext::CreateFramebuffers(VkImageView colorView,
                                              VkFramebuffer& framebuffer)
{
    ...
    framebufferInfo.layers = BATCH_LAYER_COUNT;
...
void OptimizeResourceContext::CleanupMaskResource(VkImage& image, VkImageView& view,
...
void OptimizeResourceContext::UpdateGlobalSSBO(void* SSBOData, size_t size)
{
    m_cameraSSBOBuffer->WriteToBuffer(SSBOData, size);
    m_cameraSSBOBuffer->Flush();
}
