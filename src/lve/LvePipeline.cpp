...
LvePipeline::~LvePipeline()
{
    ...
    if (m_geomShaderModule != VK_NULL_HANDLE) {
        vkDestroyShaderModule(m_lveDevice.device(), m_geomShaderModule, nullptr);
    }
                                         ...
                                         const std::string& geomFilepath)
{
    ...
    if (!geomFilepath.empty()) {
        ...
        CreateShaderModule(geomCode, &m_geomShaderModule);
    }
    ...
    vertexShaderStage.stage = VK_SHADER_STAGE_VERTEX_BIT;  // 顶点着色器
    vertexShaderStage.module = m_vertShaderModule;  // 绑定顶点着色器的VkShaderModule
    vertexShaderStage.pName = "main";  // SPIR-V 的 entry point 名称（GLSL 默认
                                     // main，若编译时改过，这里也要一致）
    ...
    if (m_geomShaderModule != VK_NULL_HANDLE) {
        ...
        geomShaderStage.module = m_geomShaderModule;
    ...
    fragShaderStage.module = m_fragShaderModule;
    ...
    vertexInputInfo.pVertexAttributeDescriptions =
        attributeDescriptions.data();  // 应用顶点属性描述
    vertexInputInfo.pVertexBindingDescriptions =
        bindingDescription.data();  // 应用顶点绑定描述
    ...
    if (vkCreateGraphicsPipelines(m_lveDevice.device(),
                                  VK_NULL_HANDLE,
                                  1,
                                  &pipelineInfo,
                                  nullptr,
                                     ...
                                     VkShaderModule* shaderModule)
{
    ...
    if (vkCreateShaderModule(m_lveDevice.device(), &createInfo, nullptr, shaderModule) !=
        VK_SUCCESS) {
...
void LvePipeline::DefaultPipelineConfigInfo(PipelineConfigInfo& configInfo)
{
    ...
    configInfo.inputAssemblyInfo.sType =
        VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;  // 标明结构体是用于描述图元装配阶段配置的
    configInfo.inputAssemblyInfo.topology =
        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;  //每三个顶点构成一个独立三角形
    ...
    configInfo.rasterizationInfo.sType =
        VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;  //标明结构体是描述光栅化阶段配置的
    configInfo.rasterizationInfo.depthClampEnable =
        VK_FALSE;  // 关闭深度钳制（如果开启，超出近平远平面的片元会被钳到边界，而不是被裁剪掉
    configInfo.rasterizationInfo.rasterizerDiscardEnable =
        VK_FALSE;  // 不丢弃光栅化（如果开启，将不产生任何片元输出）
    configInfo.rasterizationInfo.polygonMode =
        VK_POLYGON_MODE_FILL;  // 三角形以填充方式绘制（而不是线框、点，非FILL需要GPU支持）
    configInfo.rasterizationInfo.lineWidth = 1.0f;  // 线宽1，大于1需要线宽特性？
    configInfo.rasterizationInfo.cullMode =
        VK_CULL_MODE_NONE;  // 关闭背面剔除。性能略低，但不易出现正反面反了导致看不见的坑
    configInfo.rasterizationInfo.frontFace =
        VK_FRONT_FACE_CLOCKWISE;  // 规定顺时针顶点顺序为正面
    ...
    configInfo.multisampleInfo.sType =
        VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;  // 标明结构体是描述多重采样阶段配置的
    configInfo.multisampleInfo.sampleShadingEnable = VK_FALSE;  // 关闭每样本着色
    configInfo.multisampleInfo.rasterizationSamples =
        VK_SAMPLE_COUNT_1_BIT;  // 1个样本（无MSAA），与渲染通道/附件的样本数必须匹配
    ...
    configInfo.colorBlendInfo.sType =
        VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO;  // 颜色混合状态
    configInfo.colorBlendInfo.logicOpEnable =
        VK_FALSE;  // 关闭逻辑运算，按位运算式混色方式
    configInfo.colorBlendInfo.logicOp = VK_LOGIC_OP_COPY;  // Optional
    configInfo.colorBlendInfo.attachmentCount = 1;  // 只有1个颜色附件需要配置混合
    configInfo.colorBlendInfo.pAttachments =
        &configInfo.colorBlendAttachment;  // 指向colorBlendAttachment的颜色混合
    ...
    configInfo.depthStencilInfo.sType =
        VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;  // 深度/模板状态
    configInfo.depthStencilInfo.depthTestEnable = VK_TRUE;           // 开启深度测试
    configInfo.depthStencilInfo.depthWriteEnable =
        VK_TRUE;  // 通过深度测试的片元会写入深度缓冲
    configInfo.depthStencilInfo.depthCompareOp =
        VK_COMPARE_OP_LESS;  // 片元深度小于当前深度才通过（更近覆盖更远）
    configInfo.depthStencilInfo.depthBoundsTestEnable =
        VK_FALSE;  // 关闭深度区间测试（只有在做特殊裁剪时会用到）
...
{
    configInfo.colorBlendAttachment.blendEnable =
        VK_TRUE;  // 启用会有性能成本，只有需要使用alpha混合的对象才启用
