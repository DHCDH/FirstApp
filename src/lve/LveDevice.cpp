...
{
    createInstance();       // 初始化Vulkan实例
    setupDebugMessenger();  // 设置验证层（默认情况下，Vulkan几乎不做任何错误检查），在调试过程中开启验证层，发布版本中再禁用
    createSurface();        // 创建依赖GLFW的窗口
    pickPhysicalDevice();  // 选择物理设备（系统中能与Vulkan协同工作的设备（GPU））
    createLogicalDevice();  // 创建逻辑设备（描述了希望使用物理设备的哪些功能）
...
void LveDevice::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
