...
{
public:
    FirstApp(void* nativeWindowHandle, void* nativeInstanceHandle, int w, int h,
             std::string name);
    ~FirstApp();
    ...
    lve::LveWindow* GetLveWindow() const
    {
        return m_lveWindow.get();
    }
    ...
    {
        if (!m_grindingWheel) return;
        m_grindingWheel->SetMotionEnabled(enabled);
    }
    void SetInstancesShown(const bool& shown)
    {
...
private:
    ...
    std::unique_ptr<lve::LveDescriptorSetLayout>
        m_materialParamSetLayout;  // 仅 binding=0: UBO
    std::unordered_map<uint32_t, lve::MaterialGPU> m_objectMaterialParams;
    std::unordered_map<uint32_t, std::vector<VkDescriptorSet>>
        m_submeshTextureSets;  // 材质贴图表，set = 1；[objId] = submeshId
    VkDescriptorSet
        m_defaultTextureSet{};  // 默认贴图，当某个obj的submesh没有配置贴图时，仍保证set=1一定被绑定

    std::unordered_map<
        uint32_t,
        std::vector<std::array<VkDescriptorSet, lve::LveSwapChain::MAX_FRAMES_IN_FLIGHT>>>
        m_submeshMatSets;  // 材质参数UBO表，set = 2；
    ...
    void UpdateMaterialParamsPerFrame(uint32_t objId, int frameIndex,
                                      const lve::MaterialUBO& data);
