...
namespace entity
{
void Entity::CreateMaterialParamSetsForSubmesh(uint32_t objId, uint32_t submeshIndex,
                                               const lve::MaterialUBO& u)
{
    ...
    for (int i = 0; i < lve::LveSwapChain::MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        lve::LveDescriptorWriter(m_renderContext.materialParamSetLayout,
                                 m_renderContext.materialParamPool)
            .WriteBuffer(0, &bi)
            .Build(slot[i]);
